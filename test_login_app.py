...
class TestAdditionalRegressionCases:
    ...
    def test_hash_password_collision_resistance(self):
        """Test that different passwords produce different hashes"""
        hash1 = login_app.hash_password("password1")
        hash2 = login_app.hash_password("password2")
        hash3 = login_app.hash_password("password3")
        assert hash1 != hash2
        assert hash2 != hash3
        assert hash1 != hash3

    @patch("builtins.open", new_callable=mock_open)
    def test_add_user_with_whitespace_username(self, mock_file):
        """Test adding user with whitespace in username"""
        login_app.add_user("user name", "password")
        assert any(u["username"] == "user name" for u in login_app.USERS)

    @patch("builtins.open", new_callable=mock_open)
    def test_add_user_with_leading_trailing_spaces(self, mock_file):
        """Test adding user with leading/trailing spaces"""
        login_app.add_user("  admin  ", "password")
        # Function doesn't trim spaces
        assert any(u["username"] == "  admin  " for u in login_app.USERS)

    @patch("builtins.print")
    @patch("random.randint")
    def test_session_overflow_behavior(self, mock_random, mock_print):
        """Test behavior with many concurrent sessions"""
        mock_random.side_effect = range(1000, 1100)
        for i in range(100):
            login_app.login("admin", "1234")
        # All sessions should be preserved (no expiration)
        assert len(login_app.SESSIONS) >= 100

    @patch("builtins.open", new_callable=mock_open)
    def test_reset_password_to_same_password(self, mock_file):
        """Test resetting password to the same value"""
        result = login_app.reset_password("admin", "1234")
        assert result is True
        admin_user = next(u for u in login_app.USERS if u["username"] == "admin")
        assert admin_user["password"] == "1234"

    @patch("builtins.open", new_callable=mock_open)
    def test_reset_password_with_special_characters(self, mock_file):
        """Test resetting password to value with special characters"""
        new_pass = "p@$$w0rd!#%^&*()"
        result = login_app.reset_password("admin", new_pass)
        assert result is True
        admin_user = next(u for u in login_app.USERS if u["username"] == "admin")
        assert admin_user["password"] == new_pass

    @patch("builtins.print")
    def test_login_with_whitespace_password(self, mock_print):
        """Test login with password containing only whitespace"""
        login_app.USERS.append({"username": "spaceuser", "password": "   "})
        result = login_app.login("spaceuser", "   ")
        assert result is True

    @patch("builtins.open", mock_open(read_data='{"nested": {"array": [1, 2, 3]}}'))
    def test_read_config_with_arrays(self):
        """Test reading config with array values"""
        result = login_app.read_config("config.json")
    ...
    def test_hash_password_deterministic(self):
        ...
        hash3 = login_app.hash_password(password)
        assert hash1 == hash2 == hash3

    @patch("builtins.print")
    @patch("random.randint", side_effect=[1000, 1000])
    def test_session_token_collision_overwrites(self, mock_random, mock_print):
        """Test that colliding session tokens overwrite previous sessions"""
        login_app.login("admin", "1234")
        first_user = login_app.SESSIONS["1000"]["username"]
        login_app.login("test", "test123")
        second_user = login_app.SESSIONS["1000"]["username"]
        # Second login overwrites first due to collision
        assert first_user == "admin"
        assert second_user == "test"

    @patch("builtins.open", new_callable=mock_open)
    def test_add_user_unicode_password(self, mock_file):
        """Test adding user with unicode characters in password"""
        login_app.add_user("user", "密码123")
        user = next(u for u in login_app.USERS if u["username"] == "user")
        assert user["password"] == "密码123"

    @patch("builtins.print")
    def test_login_case_sensitivity_verification(self, mock_print):
        """Verify login is case-sensitive for both username and password"""
        # Add a user with mixed-case password
        login_app.USERS.append({"username": "testcase", "password": "PassWord"})

        # Correct case should work
        result1 = login_app.login("testcase", "PassWord")
        assert result1 is True

        # Wrong case for username should fail
        result2 = login_app.login("TESTCASE", "PassWord")
        assert result2 is False

        # Wrong case for password should fail
        result3 = login_app.login("testcase", "password")
        assert result3 is False

    def test_hash_password_with_newlines(self):
        """Test hashing password with newline characters"""
        result = login_app.hash_password("pass\nword")
        assert isinstance(result, str)
        assert len(result) == 32

    @patch("builtins.open", new_callable=mock_open)
    @patch("builtins.print")
    def test_add_multiple_users_rapidly(self, mock_print, mock_file):
        """Test adding multiple users in rapid succession"""
        initial_count = len(login_app.USERS)
        for i in range(50):
            login_app.add_user(f"user{i}", f"pass{i}")
        assert len(login_app.USERS) == initial_count + 50

    @patch("builtins.open", new_callable=mock_open)
    @patch("builtins.print")
    def test_user_lifecycle_complete(self, mock_print, mock_file):
        """Complete user lifecycle test"""
        # Add user
        login_app.add_user("lifecycle", "initial")
        assert any(u["username"] == "lifecycle" for u in login_app.USERS)

        # Login with initial password
        result = login_app.login("lifecycle", "initial")
        assert result is True

        # Reset password
        login_app.reset_password("lifecycle", "changed")
        ...
        # New password should work
        result = login_app.login("lifecycle", "changed")
        assert result is True