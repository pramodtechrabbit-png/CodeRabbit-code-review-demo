...
class TestAdditionalRegressionCases:
    ...
    @patch("sqlite3.connect")
    def test_database_connection_failure_handling(self, mock_connect):
    ...
    @patch("sqlite3.connect")
    def test_add_user_db_with_sql_special_characters(self, mock_connect):
        """Test adding user with SQL-like special characters"""
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_connect.return_value = mock_conn
        mock_conn.cursor.return_value = mock_cursor

        buggy_login_app.add_user_db("user'; DROP TABLE users;--", "password")
        # Verify parameterized query protects against SQL injection
        assert mock_cursor.execute.called

    def test_session_token_uniqueness_collision_scenario(self):
        """Test session token collision scenario"""
        with patch("random.randint", return_value=5000):
            token1 = buggy_login_app.create_session("user1")
            token2 = buggy_login_app.create_session("user2")
            # Second session overwrites first due to collision
            assert buggy_login_app.SESSIONS["5000"]["username"] == "user2"

    def test_calculate_total_with_negative_quantities(self):
        """Test calculate_total with negative quantities (refunds/returns)"""
        items = [
            {"price": 100.0, "quantity": -2},
            {"price": 50.0, "quantity": 1}
        ]
    ...
    def test_apply_discount_boundary_at_100_percent(self):
        """Boundary test: 100% discount should result in zero"""
        result = buggy_login_app.apply_discount(100.0, 100)
        assert result == 0.0

    def test_apply_discount_boundary_at_0_percent(self):
        """Boundary test: 0% discount should return original amount"""
        result = buggy_login_app.apply_discount(100.0, 0)
        assert result == 100.0

    @patch("builtins.open", new_callable=mock_open)
    def test_concurrent_add_user_operations(self, mock_file):
        """Test concurrent user additions"""
        initial_count = len(buggy_login_app.USERS)
        buggy_login_app.add_user("user1", "pass1")
        buggy_login_app.add_user("user2", "pass2")
        buggy_login_app.add_user("user3", "pass3")
        assert len(buggy_login_app.USERS) == initial_count + 3

    def test_hash_password_with_very_long_input(self):
        """Test hashing extremely long password"""
        long_password = "a" * 100000
        result = buggy_login_app.hash_password(long_password)
        assert len(result) == 32

    def test_generate_invoice_date_format(self):
        """Test that invoice contains valid date format"""
        items = [{"price": 10.0, "quantity": 1}]
        invoice = buggy_login_app.generate_invoice("INV-001", items)
        assert "date" in invoice
        assert isinstance(invoice["date"], str)
        assert len(invoice["date"]) > 0

    @patch("buggy_login_app.authenticate", return_value=True)
    @patch("random.randint", return_value=9999)
    @patch("builtins.print")
    def test_login_creates_session_with_time_attribute(self, mock_print, mock_random, mock_auth):
        """Verify session includes timestamp"""
        buggy_login_app.login("user", "pass")
        session = buggy_login_app.SESSIONS["9999"]
        assert "time" in session
        assert isinstance(session["time"], float)

    @patch("builtins.open", new_callable=mock_open)
    def test_write_config_with_nested_structures(self, mock_file):
        """Test writing config with deeply nested structures"""
        data = {
            "level1": {
                "level2": {
                    "level3": {
    ...
    def test_calculate_total_floating_point_precision(self):
        """Test floating-point precision in total calculation"""
        items = [
        ...
        assert abs(total - 0.9) < 0.0001

    @patch("builtins.open", new_callable=mock_open)
    def test_delete_user_empty_list_behavior(self, mock_file):
        """Test deleting users until list is empty"""
        buggy_login_app.USERS.clear()
        initial_users = [
            {"username": "user1", "password": "pass1"},
            {"username": "user2", "password": "pass2"}
        ]
        buggy_login_app.USERS.extend(initial_users)

        buggy_login_app.delete_user("user1")
        buggy_login_app.delete_user("user2")
        assert len(buggy_login_app.USERS) == 0

    def test_list_users_returns_mutable_reference(self):
        """Test that list_users returns direct reference (security issue)"""
        users = buggy_login_app.list_users()
        original_len = len(buggy_login_app.USERS)
        users.append({"username": "hacker", "password": "hack"})
        # Direct modification affects the original list
        assert len(buggy_login_app.USERS) == original_len + 1