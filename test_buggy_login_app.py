...
class TestRegressionAndBoundary:
    ...
    def test_session_expiration_not_implemented(self):
        """Regression test: Document that session expiration is not implemented"""
        buggy_login_app.SESSIONS.clear()
        with patch("random.randint", return_value=9999):
            token = buggy_login_app.create_session("testuser")
            session = buggy_login_app.SESSIONS[token]

            # Session has a timestamp but no expiration check exists
            assert "time" in session
            assert isinstance(session["time"], float)

            # Simulate time passing - session remains valid indefinitely
            import time
            old_time = session["time"]
            time.sleep(0.1)
            # Session is still in SESSIONS with old timestamp - no cleanup mechanism
            assert token in buggy_login_app.SESSIONS
            assert buggy_login_app.SESSIONS[token]["time"] == old_time
