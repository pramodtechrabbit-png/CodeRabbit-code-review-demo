...
class TradeProcessorTest {
    ...
    @Test
    @DisplayName("Regression: processTrade with null consumer")
    void testProcessTradeWithNullConsumer() {
        TradeProcessor processorWithNull = new TradeProcessor(null);
        assertDoesNotThrow(() -> processorWithNull.processTrade("TRADE-001"));
    }

    @Test
    @DisplayName("Regression: calculatePnL with minimum double values")
    void testCalculatePnLMinimumValues() {
        double result = tradeProcessor.calculatePnL(Double.MIN_VALUE, Double.MIN_VALUE, 1);
        assertEquals(0.0, result, 0.001);
    }

    @Test
    @DisplayName("Negative test: cacheTrade then clearCache then cacheTrade")
    void testCacheClearCacheCycleMultipleTimes() {
        for (int i = 0; i < 5; i++) {
            tradeProcessor.cacheTrade("TRADE-" + i);
            tradeProcessor.clearCache();
        }
    ...
    @Test
    @DisplayName("Regression: calculatePnL precision with many small transactions")
    void testCalculatePnLPrecisionAccumulation() {
        double sum = 0.0;
        for (int i = 0; i < 1000; i++) {
            sum += tradeProcessor.calculatePnL(100.00, 100.01, 1);
        }
        // Each transaction gains 0.01, so 1000 should be 10.0
        assertTrue(Math.abs(sum - 10.0) < 0.1);
    }

    @Test
    @DisplayName("Negative test: startConsumer with null records")
    void testStartConsumerHandlesNullRecords() throws InterruptedException {
        when(mockConsumer.poll(any(Duration.class)))
            .thenReturn(null)
            .thenThrow(new RuntimeException("Stop loop"));
    ...
    @Test
    @DisplayName("Boundary test: processTrade with maximum integer string")
    void testProcessTradeWithVeryLargeNumber() {
        String largeNumber = String.valueOf(Long.MAX_VALUE);
        assertDoesNotThrow(() -> tradeProcessor.processTrade("TRADE-" + largeNumber));
    }
}